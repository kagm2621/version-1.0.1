<?php
	...
	*/
	...
	add_action('gdlr_print_item_selector', 'gdlrs_check_room_item', 10, 2);
	if( !function_exists('gdlrs_check_room_item') ){
		function gdlrs_check_room_item( $type, $settings = array() ){
			if($type == 'hostel-room'){
				echo gdlrs_print_room_item( $settings );
			}else if($type == 'hostel-room-category'){
	...
	// print room item
	if( !function_exists('gdlrs_print_room_category_item') ){
		function gdlrs_print_room_category_item( $settings = array() ){
			...
			if( empty($settings['category']) ){
				$room_branches = get_categories( array('taxonomy'=>'hostel_room_category', 'hide_empty'=>1) );
			}else{
				$room_branches = get_categories( array('taxonomy'=>'hostel_room_category', 'include'=>explode(',', $settings['category'])) );
			}
	...
	// print room item
	if( !function_exists('gdlrs_print_room_item') ){
		function gdlrs_print_room_item( $settings = array() ){
			...
			if( !empty($settings['category']) || !empty($settings['tag']) ){
				...
				if( !empty($settings['category']) ){
					array_push($args['tax_query'], array('terms'=>explode(',', $settings['category']), 'taxonomy'=>'hostel_room_category', 'field'=>'slug'));
				}
				if( !empty($settings['tag']) ){
					array_push($args['tax_query'], array('terms'=>explode(',', $settings['tag']), 'taxonomy'=>'hostel_room_tag', 'field'=>'slug'));
				}				
			}			
			...
			if( ($settings['room-style'] == 'classic' || $settings['room-style'] == 'modern') &&
				!empty($settings['enable-carousel']) && $settings['enable-carousel'] == 'enable' ){
			...
			$ret .= '<div class="room-item-holder">';
			if( $settings['room-style'] == 'medium' ){
				...
				$ret .= gdlrs_get_medium_room($query, $settings['thumbnail-size']);
				...
				if( !empty($settings['enable-carousel']) && $settings['enable-carousel'] == 'enable' ){
					$ret .= gdlrs_get_classic_room_carousel($query, $settings['room-size'], $settings['thumbnail-size']);
				}else{
					$ret .= gdlrs_get_classic_room($query, $settings['room-size'], $settings['thumbnail-size']);
				}
	...
	// get room style
	if( !function_exists('gdlrs_get_medium_room') ){
		function gdlrs_get_medium_room($query, $thumbnail_size){
			$ret  = '';
			while($query->have_posts()){ $query->the_post();
				...
				$ret .= '<a class="gdlr-button with-border" href="' . get_permalink() . '">' . __('Ver Detalles', 'gdlr-hostel') . '<i class="fa fa-long-arrow-right icon-long-arrow-right"></i></a>';
	...
	if( !function_exists('gdlrs_get_classic_room') ){
		function gdlrs_get_classic_room($query, $size, $thumbnail_size){
			$current_size = 0; $ret  = '';
			while($query->have_posts()){ $query->the_post();
				$post_option = json_decode(gdlr_decode_preventslashes(get_post_meta(get_the_ID(), 'post-option', true)), true);
	...
	if( !function_exists('gdlrs_get_classic_room_carousel') ){
	...
	if( !function_exists('gdlr_get_modern_room_carousel') ){
		function gdlr_get_modern_room_carousel($query, $size, $thumbnail_size){
			...
			while($query->have_posts()){ $query->the_post();
				...
				$ret .= '<div class="gdlr-room-thumbnail">' . gdlr_get_room_thumbnail($post_option, $thumbnail_size) . '</div>';
				$ret .= '<h3 class="gdlr-room-title"><a href="' . get_permalink() . '" >' . get_the_title() . '</a></h3>';
				$ret .= '<a href="' . get_permalink() . '" class="gdlr-room-detail">' . __('Ver Detalles', 'gdlr-hostel') . '<i class="fa fa-long-arrow-right icon-long-arrow-right"></i></a>';
				$ret .= '</li>'; // gdlr-item
			}
			$ret .= '</ul>';
			$ret .= '<div class="clear"></div>';
	...
	if( !function_exists('gdlr_hotel_room_info_new') ){
		function gdlr_hotel_room_info_new( $options = array(), $list = array(), $wrapper = true ){
			$ret  = '';
			$max_count = 99;
			if( !empty($list) ){
				$max_count = sizeOf($list);
			}
			$count = 1;
	...
	if( !function_exists('gdlr_hostel_room_info') ){
				...
				switch( $slug ){
					...
					case 'price-break-down': 
						...
						if( !empty($post_option['room-base-price']) ){
							...
							if( $slug == 'price' || empty($hostel_option['booking-price-display']) || $hostel_option['booking-price-display'] == 'start-from' ){
								$ret .= '<span class="gdlr-head">' . __('Start From', 'gdlr-hostel') . '</span>';
								$ret .= '<span class="gdlr-tail">' . gdlr_hostel_money_format($post_option['room-base-price']) . ' / ';
							}else{
								$price_breakdown = get_hostel_price_breakdown_popup($post_option);
								$ret .= '<span class="gdlr-tail">' . gdlr_hostel_money_format($price_breakdown['total']) . ' / ';
							}
							if(!empty($post_option['room-type']) && $post_option['room-type'] == 'private'){
								$ret .= __('Room', 'gdlr-hostel');
							}else{
								$ret .= __('Guest', 'gdlr-hostel');
							}
							$ret .= '</span>';
							if( $slug == 'price-break-down' ){
								...
								$ret .= '<div class="gdlr-price-break-down" >' . __('* view price breakdown', 'gdlr-hostel');
					...
					case 'bed': 
						if( !empty($post_option[$slug]) ){
							$ret .= '<div class="gdlr-room-info"  style="background-color: #252525;">';
							$ret .= '<i class="fa fa-check-square-o icon-check" ></i>';
							$ret .= '<span class="gdlr-head">' . __('Bed', 'gdlr-hostel') . '</span>';
					...
					case 'max-people':  
						if( !empty($post_option[$slug]) ){
							$ret .= '<div class="gdlr-room-info"  style="background-color: #252525;">';
							$ret .= '<i class="fa fa-check-square-o icon-check" ></i>';
							$ret .= '<span class="gdlr-head">' . __('Max', 'gdlr-hostel') . '</span>';
							$ret .= '<span class="gdlr-tail">' . $post_option[$slug] . ' ' . __('People', 'gdlr-hostel') . '</span>';
							$ret .= '</div>';
						}
						break;
					case 'view':  
						if( !empty($post_option[$slug]) ){
							$ret .= '<div class="gdlr-room-info">';
							$ret .= '<i class="fa fa-check-square-o icon-check" ></i>';
							$ret .= '<span class="gdlr-head">' . __('View', 'gdlr-hostel') . '</span>';
							$ret .= '<span class="gdlr-tail">' . $post_option[$slug] . '</span>';
							$ret .= '</div>';
						}
						break;
					case 'room-size':   
						if( !empty($post_option[$slug]) ){
							$ret .= '<div class="gdlr-room-info">';
							$ret .= '<i class="fa fa-check-square-o icon-check" ></i>';
							$ret .= '<span class="gdlr-head">' . __('Room Size', 'gdlr-hostel') . '</span>';
							$ret .= '<span class="gdlr-tail">' . $post_option[$slug] . '</span>';
							$ret .= '</div>';
						}
						break;
					case 'wifi':   
						if( !empty($post_option[$slug]) ){
							$ret .= '<div class="gdlr-room-info">';
							$ret .= '<i class="fa fa-check-square-o icon-check" ></i>';
							$ret .= '<span class="gdlr-head">' . __('Wifi', 'gdlr-hostel') . '</span>';
							$ret .= '<span class="gdlr-tail">' . $post_option[$slug] . '</span>';
							$ret .= '</div>';
						}
						break;
					case 'breakfast-included':  
						if( !empty($post_option[$slug]) ){
							$ret .= '<div class="gdlr-room-info">';
							$ret .= '<i class="fa fa-check-square-o icon-check" ></i>';
							$ret .= '<span class="gdlr-head">' . __('Desayuno incluido', 'gdlr-hostel') . '</span>';
							$ret .= '<span class="gdlr-tail">' . $post_option[$slug] . '</span>';
							$ret .= '</div>';
						}
						break;
					case 'room-service':   
						if( !empty($post_option[$slug]) ){
							$ret .= '<div class="gdlr-room-info">';
							$ret .= '<i class="fa fa-check-square-o icon-check" ></i>';
							$ret .= '<span class="gdlr-head">' . __('Servicios de habitación', 'gdlr-hostel') . '</span>';
							$ret .= '<span class="gdlr-tail">' . $post_option[$slug] . '</span>';
							$ret .= '</div>';
						}
						break;
					case 'airport-pickup-service':  
						if( !empty($post_option[$slug]) ){
							$ret .= '<div class="gdlr-room-info">';
							$ret .= '<i class="fa fa-check-square-o icon-check" ></i>';
							$ret .= '<span class="gdlr-head">' . __('Airport Pickup Service', 'gdlr-hostel') . '</span>';
							$ret .= '<span class="gdlr-tail">' . $post_option[$slug] . '</span>';
							$ret .= '</div>';
						}
						break;
