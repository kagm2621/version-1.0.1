	...
	*/
	...
	if( !function_exists('gdlr_booking_process_bar') ){
		function gdlr_booking_process_bar( $state = 1 ){
			...
			$ret .= '<div data-process="1" class="gdlr-booking-process ' . (($state==1)? 'gdlr-active': '') . '">' . __('1. Fecha', 'gdlr-hostel') . '</div>';
			$ret .= '<div data-process="2" class="gdlr-booking-process ' . (($state==2)? 'gdlr-active': '') . '">' . __('2. Habitación', 'gdlr-hostel') . '</div>';
			$ret .= '<div data-process="3" class="gdlr-booking-process ' . (($state==3)? 'gdlr-active': '') . '">' . __('3. Reservación', 'gdlr-hostel') . '</div>';
			$ret .= '<div data-process="4" class="gdlr-booking-process ' . (($state==4)? 'gdlr-active': '') . '">' . __('4. Confirmación', 'gdlr-hostel') . '</div>';
			$ret .= '</div>';
			return $ret;
		}
	}
	...
	if( !function_exists('gdlrs_get_booking_room_query') ){
		function gdlrs_get_booking_room_query($data, $room_number){
			...
			if( !empty($blocked_date) ){
				$ret  = '<div class="gdlr-hotel-missing-room">';
				$ret .= '<i class="fa fa-frown-o icon-frown"></i>';
				$ret .= __('Lo sentimos, Nuestro hotel esta cerrado en las siguientes fechas:', 'gdlr-hostel');
	...
	<?php
		...
		foreach( $services as $service ){
			...
			if( $option['service-type'] == 'parking-service' && $option['car'] == 'car' ){
				...
				echo '<span class="gdlr-room-service-title">' . __('cars', 'gdlr-hostel') . '</span>';
			}else{
