	...
	<form class="gdlr-transaction-table" method="post" >
		...
		<table>
			<tr>
				...
				<th><?php _e('Type', 'gdlr-hostel'); ?></th>
				<th><?php _e('Code', 'gdlr-hostel'); ?></th>
				...
				for($i=($record_per_page*($current_page - 1)); $i<$record_num && $i<($record_per_page * $current_page); $i++){ $result = $results[$i];
					...
					if( $result->payment_status == 'booking' ){
						...
						if( $result->total_price == $result->pay_amount ){
							_e('Full Amount', 'gdlr-hostel');
						}else{
							_e('Deposit', 'gdlr-hostel');
						}
					}else{
					...
					echo '<td>';
					if($status == 'new'){
						echo '<a href="#" class="gdlr-mark-as-read">' . __('Mark as read', 'gdlr-hostel') . '</a>';
					}else{
						echo '<a href="#" class="gdlr-mark-as-unread">' . __('Mark as unread', 'gdlr-hostel') . '</a>';
					}
					if( $result->payment_status == 'booking' ){
						echo ' / <a href="#" class="gdlr-mark-as-paid">' . __('Mark as paid', 'gdlr-hostel') . '</a>';
					}else{
						echo ' / <a href="#" class="gdlr-mark-as-booking">' . __('Mark as booking', 'gdlr-hostel') . '</a>';
					}
					echo '</td>';
					echo '</tr>';
				}
