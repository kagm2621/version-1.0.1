	...
	*/
	...
	if( !function_exists('gdlr_get_reservation_bar') ){
		function gdlr_get_reservation_bar($single_form = false){
			...
			$ret .= ' >';
			$ret .= '<div class="gdlr-reservation-bar-title">' . __('Tu Reservación', 'gdlr-hotel') . '</div>';
	...
	if( !function_exists('gdlr_get_summary_form') ){
		function gdlr_get_summary_form($data, $with_form = true, $coupon = ''){
			...
			if( !empty($data['gdlr-hotel-branches']) ){
				...
				$ret .= '<div class="gdlr-price-summary-head">' . __('Desglose de Precios', 'gdlr-hotel') . '</div>';
			}
			...
			for($i=0; $i<intval($data['gdlr-room-number']); $i++){
				...
				$ret .= '<span>' . __('Adultos', 'gdlr-hotel') . ' : ' . $data['gdlr-adult-number'][$i] . '</span>';
				$ret .= '<span>' . __('Niños', 'gdlr-hotel') . ' : ' . $data['gdlr-children-number'][$i] . '</span>';
				$ret .= '</div>';			
				$ret .= '</div>';
	...
	if( !function_exists('gdlr_get_reservation_room') ){
		function gdlr_get_reservation_room($option){
			...
			$ret .= '<span>' . __('Adultos', 'gdlr-hotel') . ' : ' . $option['adult'] . '</span>';
			$ret .= '<span>' . __('Niños', 'gdlr-hotel') . ' : ' . $option['children'] . '</span>';
			$ret .= empty($option['room-id'])? '': '<a data-room="' .  '" class="gdlr-reservation-change-room" href="#" >' . __('Cambiar habitación', 'gdlr-hotel') . '</a>';
			$ret .= '</div>';
			$ret .= '</div>';
	...
	if( !function_exists('gdlr_get_reservation_date_form') ){
		function gdlr_get_reservation_date_form($single_form = false, $data = array()){
			...
			// room
			$ret .= gdlr_get_reservation_combobox(array(
				'title'=>__('Habitaciones', 'gdlr-hotel'),
			...
			if( $single_form ){
				...
				$ret .= '<input type="submit" class="gdlr-reservation-bar-button gdlr-button with-border" value="' . __('Verificar Disponibilidad', 'gdlr-hotel') . '" >';
			}else if( empty($_POST['hotel_data']) ){
				$ret .= '<a id="gdlr-reservation-bar-button" class="gdlr-reservation-bar-button gdlr-button with-border" href="#" >' . __('Verificar Disponibilidad', 'gdlr-hotel') . '</a>';
			}
			$ret .= '<div class="clear"></div>';
			return $ret;
		}
