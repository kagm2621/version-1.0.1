	...
	*/
	...
	if( !function_exists('gdlr_booking_process_bar') ){
		function gdlr_booking_process_bar( $state = 1 ){
			...
			$ret .= '<div data-process="1" class="gdlr-booking-process ' . (($state==1)? 'gdlr-active': '') . '">' . __('1. Fecha', 'gdlr-hotel') . '</div>';
			$ret .= '<div data-process="2" class="gdlr-booking-process ' . (($state==2)? 'gdlr-active': '') . '">' . __('2. Habitacion', 'gdlr-hotel') . '</div>';
			$ret .= '<div data-process="3" class="gdlr-booking-process ' . (($state==3)? 'gdlr-active': '') . '">' . __('3. Reservacion', 'gdlr-hotel') . '</div>';
			$ret .= '<div data-process="4" class="gdlr-booking-process ' . (($state==4)? 'gdlr-active': '') . '">' . __('4. Confirmacion', 'gdlr-hotel') . '</div>';
			$ret .= '</div>';
			return $ret;
		}
	}
		...
		function gdlr_ajax_hotel_booking(){	
			...
			if( $_POST['state'] == 2 ){
				...
				if( empty($data['gdlr-check-in']) || empty($data['gdlr-check-out']) || $data['gdlr-check-out'] < $data['gdlr-check-in'] ){
					...
					$ret['content'] .= '<div class="gdlr-room-selection-title" >' . __('Campo de fecha invalido', 'gdlr-hotel') . '</div>';
					$ret['content'] .= '<div class="gdlr-room-selection-content" >' . __('Porfavor seleciona una \'entrada\' y \'salida\' de la barra de reservación de nuevo.', 'gdlr-hotel') . '</div>';
					$ret['content'] .= '</div>';
					...
					$ret['content']  = '<div class="gdlr-room-selection-complete">';
					$ret['content'] .= '<div class="gdlr-room-selection-title" >' . __('Selecion de habitación completada', 'gdlr-hotel') . '</div>';
					$ret['content'] .= '<div class="gdlr-room-selection-caption" >' . __('Puedes editar el libro de reservacion usando el panel de la izquierda', 'gdlr-hotel') . '</div>';
					...
					$ret['content'] .= '<a class="gdlr-button with-border gdlr-room-selection-next">' . __('Ir al siguiente paso', 'gdlr-hotel') . '</a>';
					$ret['content'] .= '</div>';
				}
	...
	if( !function_exists('gdlr_get_booking_room_query') ){
		function gdlr_get_booking_room_query($data, $room_number){
			...
			if( $data['gdlr-check-in'] == $data['gdlr-check-out'] ){
				...
				$ret .= __('Fecha ingresada incorrecta, por favor, introduzca el una fecha correcta para proceder', 'gdlr-hotel');
				$ret .= '</div>';
				return $ret;
			}
	...
	if( !function_exists('gdlr_get_booking_room') ){
		function gdlr_get_booking_room($query, $data){
			...
			while($query->have_posts()){ $query->the_post();
				...
				$ret .= 'data-roomid="' . $room_id . '" >' . __('Selecionar esta habitación', 'gdlr-hotel') . '</a>';
...
<div class="gdlr-booking-service-wrapper">
	<span class="gdlr-booking-service-head" ><?php _e('Seleciona si quieres un servicio adicional. (optional)', 'gdlr-hotel'); ?></span> 
	<form class="gdlr-booking-service-form" method="post" data-ajax="<?php echo AJAX_URL; ?>" >
	<?php
		$words = array(
			'night' => __('Noche', 'gdlr-hotel'),
			'trip' => __('Viaje', 'gdlr-hotel'),
			'car' => __('Carro', 'gdlr-hotel'),
			'group' => __('Grupo', 'gdlr-hotel'),
			'guest' => __('Invitado', 'gdlr-hotel'),
			'room' => __('Habitacion', 'gdlr-hotel'),
		);
...
<div class="gdlr-booking-contact-container">
	<form class="gdlr-booking-contact-form" method="post" data-ajax="<?php echo AJAX_URL; ?>" >
		<p class="gdlr-form-half-left">
			<span><?php _e('Nombres *', 'gdlr-hotel'); ?></span>
			<input style="color:#CEC8A2; background-color: #212121;" type="text" name="first_name" value="" />
		</p>
		<p class="gdlr-form-half-right">
			 <span><?php _e('Apellidos *', 'gdlr-hotel'); ?></span>
			 <input style="color:#CEC8A2; background-color: #212121;" type="text" name="last_name" value="" />
		</p>
		<div class="clear"></div>
		<p class="gdlr-form-half-left">
			<span><?php _e('Correo eléctronico *', 'gdlr-hotel'); ?></span>
			<input style="color:#CEC8A2; background-color: #212121;" type="text" name="email" value="" />
		</p>
		<p class="gdlr-form-half-right">
			 <span><?php _e('Teléfono *', 'gdlr-hotel'); ?></span>
			 <input style="color:#CEC8A2; background-color: #212121;" type="text" name="phone" value="" />
		</p>
		<div class="clear"></div>
		<p class="gdlr-form-half-left">
			<span><?php _e('Dirección', 'gdlr-hotel'); ?></span>
			<textarea style="color:#CEC8A2; background-color: #212121;" name="address" ></textarea>
		</p>
		<p class="gdlr-form-half-right">
			<span><?php _e('Apuntes extras', 'gdlr-hotel'); ?></span>
			<textarea  style="color:#CEC8A2; background-color: #212121;" name="additional-note" ></textarea>
		</p>
		<div class="clear"></div>
		<p class="gdlr-form-coupon">
			<span><?php _e('Codigo de descuento', 'gdlr-hotel'); ?></span>
			<input style="color:#CEC8A2; background-color: #212121;" type="text" name="coupon" value="" />
		</p>
		<div class="clear"></div>
		<div class="gdlr-error-message"></div>
		
		<a class="gdlr-button with-border gdlr-booking-contact-submit"><?php _e('Reservar ahora por correo electrónico y nos pondremos en contacto con usted.', 'gdlr-hotel'); ?></a>
		...
		<?php 
			if( $hotel_option['payment-method'] == 'instant' ){ 
				...
				echo '<a class="gdlr-button with-border gdlr-booking-payment-submit">' . __('Pagar ahora', 'gdlr-hotel') . '</a>';
			}
		?>		
	</form>
</div>
	...
	if( !function_exists('gdlr_booking_complete_message') ){
		function gdlr_booking_complete_message(){
			...
			if( !empty($_GET['response_code']) && !empty($_GET['response_reason_text']) ){
				...
				$ret .= __('Reservación completada!', 'gdlr-hotel');
				$ret .= '</div>';
				
				$ret .= '<div class="gdlr-booking-complete-caption" >';
				$ret .= __('Sus detalles de la reserva acaban de ser enviado a su correo electrónico. Si usted tiene alguna pregunta, por favor, no dude en ponerse en contacto con nosotros. ¡Gracias!', 'gdlr-hotel'); 
				$ret .= '</div>';
